...
7. Confirm settlement on-chain with the hash from that header. `tx fetch result` takes it as
   ...
   Check it against the challenge you read in step 2. Then re-read both balances. The asset balance
   from step 4 should be down by exactly the challenge's `amount`, and the native balance should be
   unchanged when `areFeesSponsored` is true:
